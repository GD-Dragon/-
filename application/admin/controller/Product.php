...
class Product extends Common
{
	...
	public function addAction()
	{
		...
		$arr=Db::query("select * from product where goods_attr_id='$goods_attr_id'");
		if (empty($arr)) {
			$arr1=Db::query("insert into product (`goods_id`,`price`,`sn_number`,`goods_attr_id`) value ('$goods_id','$price','$sn_number','$goods_attr_id')");
	...
	{   
		$data=Request::post();
		$goods_id=$data['goods_id'];
		$arr=Db::query("select * from product where goods_id=$goods_id");
	...
	public function delete()
	{
		$data=Request::post();
		$id=$data['id'];
		$arr=Db::query("delete from product where id='$id'");
