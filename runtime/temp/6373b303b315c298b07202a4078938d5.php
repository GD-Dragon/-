<?php /*a:3:{s:86:"D:\PHPstudy\PHPTutorial\WWW\03kj\shopping\application\admin\view\attrdetails\list.html";i:1563415384;s:76:"D:\PHPstudy\PHPTutorial\WWW\03kj\shopping\application\admin\view\header.html";i:1563414502;s:77:"D:\PHPstudy\PHPTutorial\WWW\03kj\shopping\application\admin\view\foother.html";i:1562166675;}*/ ?>
 <!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="text-c">
					...
					<input type="text" name="" value="<?php echo htmlentities($attr_id); ?>" id="attr_id" hidden="">
					<input type="text" placeholder="名称" value="" class="input-text" style="width:120px" id="name">
					<select name="attr_ida" id="attr_ida">
						<option >分类</option>
					</select>
...
<script>
		...
		$.ajax({
			url:"<?php echo url('attrdetails/updateaction'); ?>",
				type:'post',
				data:{
					__token__:__token__,
					id:id,
	...
	function getCate(){
		$.ajax({
		url:'<?php echo url("attrdetails/showcate"); ?>',
		dataType:'json',
		success:function(res){
			console.log(res)
			var option=''
...
function show() {
	...
	console.log(attr_id)
	$.ajax({
		url:'<?php echo url("attrdetails/show"); ?>',
		type:'post',
		data:{
			attr_id:attr_id,
		},
		...
		success:function(res){
			...
			var tr=''
				for (var i = 0; i <data.length; i++){
					tr=tr+"<tr><td><input class='mycheckbox' type='checkbox' value='"+data[i]['id']+"'></td><td>"+data[i]['id']+"</td><td>"+data[i]['name']+"</td><td>"+data[i]['a_b_name']+"</td><td class='f-14 product-brand-manage'><a style='text-decoration:none' onclick='modaldemo(\""+data[i]['id']+"\",\""+data[i]['name']+"\")' title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a><a onclick='del("+data[i].id+")' style='text-decoration:none' class='ml-5' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a></td></tr>"	;			
				}
			$('.table tbody').html(tr)
		}
			}
...
show()
	...
	function del(id){
		...
		if (r==true){
			$.ajax({
				url:"<?php echo url('attrdetails/delete'); ?>",
				data:{
					id:id,
					 __token__:__token__,
				},
		...
		function datadel(){
			...
			if (r==true)
			{
				...
				$.ajax({
					url:"<?php echo url('attrdetails/deleteMore'); ?>",
					type:'post',
					data:{
						id:id,
						__token__:__token__,
	...
	$(document).ready(function(){
		...
		var attr_id=$('#attr_id').val()
			$.ajax({
			url:'<?php echo url("attrdetails/addaction"); ?>',
			type:'post',
			data:{
				__token__:__token__,
				name:name,
