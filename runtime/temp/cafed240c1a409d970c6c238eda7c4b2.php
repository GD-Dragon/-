<?php /*a:3:{s:84:"D:\PHPstudy\PHPTutorial\WWW\03kj\shopping\application\admin\view\attribute\list.html";i:1563418649;s:76:"D:\PHPstudy\PHPTutorial\WWW\03kj\shopping\application\admin\view\header.html";i:1563420714;s:77:"D:\PHPstudy\PHPTutorial\WWW\03kj\shopping\application\admin\view\foother.html";i:1562166675;}*/ ?>
 <!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="text-c">
					...
					<input type="text" name="" value="<?php echo htmlentities($attrcate_id); ?>" id="attrcate_id" hidden="">
					<input type="text" placeholder="名称" value="" class="input-text" style="width:120px" id="name">
					<select name="" id="attrcate_ida">
						<option value="">分类</option>
					</select>
					<button type="button" class="btn btn-success" id="add" name="" onCliack=""><i class="Hui-iconfont">&#xe600;</i> 添加</button>
...
<script>
		function modaldemo(id,name,attrcate_id){
		$("#up_id").val(id)
		$("#up_name").val(name)
		...
		var attrcate_id=$("#up_attrcate_id").val()
		var __token__=$("#__token__").val()
		$.ajax({
			url:"<?php echo url('attribute/updateaction'); ?>",
				type:'post',
				data:{
					__token__:__token__,
					id:id,
	...
	function getCate(){
		$.ajax({
		url:'<?php echo url("attribute/showcate"); ?>',
		dataType:'json',
		success:function(res){
			console.log(res)
			var option=''
...
function show() {
	...
	$.ajax({
		...
		url:'<?php echo url("attribute/show"); ?>',
		type:'post',
		data:{
			attrcate_id:attrcate_id,
		},
		...
		success:function(res){
			...
			var tr=''
				for (var i = 0; i <data.length; i++){
					tr=tr+"<tr><td><input class='mycheckbox' type='checkbox' value='"+data[i]['id']+"'></td><td>"+data[i]['id']+"</td><td>"+data[i]['name']+"</td><td>"+data[i]['a_c_name']+"</td><td><a  href='<?php echo url('attrdetails/list'); ?>?id="+data[i]['id']+"'style='text-decoration:none' class='delete_one ml-5' ><i class='Hui-iconfont'>属性&#xe717;</i></a></td><td class='f-14 product-brand-manage'><a style='text-decoration:none' onclick='modaldemo(\""+data[i]['id']+"\",\""+data[i]['name']+"\",\""+data[i]['attrcate_id']+"\")' title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a><a onclick='del("+data[i].id+")' style='text-decoration:none' class='ml-5' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a></td></tr>"	;			
				}
			$('.table tbody').html(tr)
		}
			}
...
show()
	...
	function del(id){
		...
		if (r==true){
			$.ajax({
				url:"<?php echo url('attribute/delete'); ?>",
				data:{
					id:id,
					 __token__:__token__,
				},
		...
		function datadel(){
			...
			if (r==true)
			{
				...
				$.ajax({
					url:"<?php echo url('permission/deleteMore'); ?>",
					type:'post',
					data:{
						id:id,
						__token__:__token__,
	...
	$(document).ready(function(){
		...
		var attrcate_id=$('#attrcate_id').val()
			$.ajax({
			url:'<?php echo url("attribute/addaction"); ?>',
			type:'post',
			data:{
				__token__:__token__,
				name:name,
